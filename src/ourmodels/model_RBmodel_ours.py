...
class OurModel(nn.Module):
    def __init__(self, args, shared_dim=128, sim_dim=64, k=2):
        ...
        self.dynamics_model = nn.ModuleList([NeuralDynamics(input_size=int(sim_dim / 2), hidden_size=int(sim_dim / 2)) for _ in range(4)])
    ...
    def forward(self, text, image, mask, eventi=None, label=None,
        ...
        if label is not None and len(label) - 1 > torch.sum(label) > 1:
            if mu_0 is None:
                ...
                loss_dyn = self.criterion_dyn(dynamics_pred_mu_0, dynamics_label_mu_0) + self.criterion_dyn(dynamics_pred_sigma_0, dynamics_label_sigma_0) + \
                           self.criterion_dyn(dynamics_pred_mu_1, dynamics_label_mu_1) + self.criterion_dyn(dynamics_pred_sigma_1, dynamics_label_sigma_1)
        else:
            dyn_t = torch.tensor([eventi]).to(self.args.device)
            dynamics_pred_mu_0 = self.dynamics_model[0](dyn_t, mu_0)
            dynamics_pred_sigma_0 = self.dynamics_model[1](dyn_t, sigma_0)
        ...
        gaussian_feature = torch.cat([dynamics_pred_mu_0 + dynamics_pred_sigma_0 @ torch.randn(dynamics_pred_mu_0.shape[0]).to(dynamics_pred_mu_0.device),
                                      dynamics_pred_mu_1 + dynamics_pred_sigma_1 @ torch.randn(dynamics_pred_mu_1.shape[0]).to(dynamics_pred_mu_1.device)], dim=-1)
