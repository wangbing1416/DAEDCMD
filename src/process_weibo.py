...
def write_data(flag, image, text_only):
    with open("event_labels_weibo.json", "r") as f:
            ...
            for i, l in enumerate(f.readlines()):
                ...
                if (i + 1) % 3 == 0:
                    ...
                    if len(clean_l) > 10 and line_data[0] in id:  # truncrate small length
                        ...
                        try:
                            event = event_dict[line_data[0]]
                        except:
                            event = 0  # test and evaluation datasets do not need event labels
...
def get_data(text_only, continual):
    # text_only = False
    if text_only:
        print("Text only")
        image_list = []
    ...
    if continual:
        # process continual train data list
        print('Processing continual training data streams')

        train_data_list = []
        for i in range(event_num):
            indices = [idx for idx, label in enumerate(train_data['event_label']) if label == i]
            # 构造新字典
            new_dict = {"post_text": np.array([train_data['post_text'][idx] for idx in indices]),
                        "original_post": np.array([train_data['original_post'][idx] for idx in indices]),
                        "image": [train_data['image'][idx] for idx in indices],
                        "label": np.array([train_data['label'][idx] for idx in indices]),
                        "event_label": np.array([train_data['event_label'][idx] for idx in indices]),
                        "post_id": np.array([train_data['post_id'][idx] for idx in indices]),
                        "image_id": [train_data['image_id'][idx] for idx in indices]}
            # 将新字典添加到列表中
            train_data_list.append(new_dict)
        # make the last dataset largest
        train_data_list = train_data_list[1:] + train_data_list[:1]
    else:
        train_data_list = [train_data]

    validate_data, _ = write_data("validate", image_list, text_only)
    test_data, _ = write_data("test", image_list, text_only)

    return train_data_list, validate_data, test_data


if __name__ == '__main__':
    train, validate, test = get_data(text_only=False, continual=True)
    print('finish!')
