...
from ourmodels.model_RBmodel_ours import OurModel as rbmodel
from ourmodels.model_CAFE_ours import OurModel as cafe
from ourmodels.model_BMR_ours import OurModel as bmr
from ourmodels.model_MCAN_ours import OurModel as mcan
from ourmodels.model_SAFE_ours import OurModel as safe
from ourmodels.model_GAMED_ours import OurModel as gamed

from ourmodels.model_Dynamics import NeuralDynamics
...
def main(args):
    ...
    dynamics_part = 'dynamics_model'
    optimizer_grouped_parameters = [
        {
            "params": [p for n, p in model.named_parameters() if
                       not dynamics_part in n and any(nd in n for nd in diff_part)],
            "lr": args.bert_lr
        },
        {
            "params": [p for n, p in model.named_parameters() if
                       not dynamics_part in n and not any(nd in n for nd in diff_part)],
            "lr": args.learning_rate
        },
    ]
    optimizer_grouped_dyn_parameters = [
        {
            "params": [p for n, p in model.named_parameters() if dynamics_part in n],
            "lr": args.learning_rate
        },
    ]
    optimizer = torch.optim.Adam(optimizer_grouped_parameters, eps=args.adam_epsilon)
    optimizer_dyn = torch.optim.Adam(optimizer_grouped_dyn_parameters, eps=args.adam_epsilon)

    total_epoch = 0

    mu_0, sigma_0, mu_1, sigma_1 = None, None, None, None
    best_k = 2
    record_dist = [[] for _ in range(5)]
    for eventi, train_loader in enumerate(train_loader_list):
        logger.info('Training on {}-th event'.format(eventi))
        # num_epochs = np.clip(math.ceil(len(train_loader) / 4), 10, 20)
        num_epochs = 20 if eventi == len(train_loader_list) - 1 else min(math.ceil(len(train_loader) / loader_size * 50), 20)
        for d in record_dist:
            d.append([])
        for epoch in range(num_epochs):
            ...
            for i, (train_data, train_labels, event_labels) in enumerate(tqdm.tqdm(train_loader)):
                ...
                if args.baseline == 'safe':
                    class_outputs, image_text_z, image_z, text_z, sim, mu_0, sigma_0, mu_1, sigma_1, loss_dyn, dist_list = model(
                        train_text, train_image, train_mask, eventi, train_labels, mu_0, sigma_0, mu_1, sigma_1)
                    loss = criterion(class_outputs, train_labels) + args.gamma * contrastiveLoss(image_z, text_z) + criterion(sim, train_labels)
                else:
                    class_outputs, image_text_z, image_z, text_z, mu_0, sigma_0, mu_1, sigma_1, loss_dyn, dist_list = model(
                        train_text, train_image, train_mask, eventi, train_labels, mu_0, sigma_0, mu_1, sigma_1)
                    loss = criterion(class_outputs, train_labels) + args.gamma * contrastiveLoss(image_z, text_z)

                if loss_dyn:
                    loss += args.lambd * loss_dyn
                loss.backward()
                optimizer.step()
                optimizer.zero_grad()
                optimizer_dyn.step()
                ...
                for d in range(5):
                    record_temp[d].append(torch.mean(dist_list[d]).item() if dist_list[d] is not None else 0)

            for d in range(5):
                record_dist[d][-1] = record_temp[d]
            ...
            for i, (validate_data, validate_labels, event_labels) in enumerate(validate_loader):
                ...
                validate_outputs = model(validate_text, validate_image, validate_mask,
                                         eventi=5, mu_0=mu_0, sigma_0=sigma_0, mu_1=mu_1, sigma_1=sigma_1)[0]
            ...
            logger.info('Epoch [%d/%d], Loss: %.4f, Train_Acc: %.4f, Validate_Acc: %.4f, Validate_F1: : %.4f.' %
                        (epoch + 1, num_epochs, np.mean(cost_vector), np.mean(acc_vector), validate_accuracy, validate_f1))
        ...
        for test_event_i, test_event_loader in enumerate(train_loader_list):
            for i, (validate_data, validate_labels, event_labels) in enumerate(test_event_loader):
                ...
                validate_outputs = model(validate_text, validate_image, validate_mask,
                                         eventi=test_event_i, mu_0=mu_0, sigma_0=sigma_0, mu_1=mu_1, sigma_1=sigma_1)[0]
    ...
    try:
        with pd.ExcelWriter("record_dist.xlsx", engine="openpyxl") as writer:
            def write_to_sheet(data, sheet_name):
                all_data = []  # 存储所有数据
                for d in range(len(data)):
                    for value in data[d]:
                        all_data.append([d, value])
                df = pd.DataFrame(all_data, columns=["Index", "Value"])
                df.to_excel(writer, sheet_name=sheet_name, index=False)

            # 分别处理 a, b, c 并写入不同的 sheet
            for d in range(len(record_dist)):
                write_to_sheet(record_dist[d], "Sheet_{}".format(d))

        print("Excel 文件已生成: record_dist.xlsx")
    except:
        print('fail to generate a file that save variances of gaussian distribution')

    # Test the Model
    logger.info('testing model')
    if args.baseline == 'rbmodel':
        model = rbmodel(args, k=best_k)
    elif args.baseline == 'bmr':
        model = bmr(args, k=best_k)
    elif args.baseline == 'mcan':
        model = mcan(args, k=best_k)
    elif args.baseline == 'safe':
        model = safe(args, k=best_k)
    elif args.baseline == 'gamed':
        model = gamed(args, k=best_k)
    else:
        model = cafe(args, k=best_k)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--baseline', type=str, default='rbmodel', help='rbmodel, cafe, bmr, mcan, safe, gamed')
    ...
    parser.add_argument('--distribution_dim', type=int, default=8, help='')
    ...
    parser.add_argument('--lambd', type=float, default=1.0, help='balanced cof')
    ...
    args.id = '{}-{}-s{}-{}.log'.format(args.dataset, args.baseline + 'ours', seed, strftime("%m%d-%H%M", localtime()))
