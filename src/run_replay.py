...
def main(args):
    ...
    #     dataset = Rumor_Data(train)
    #     if len(dataset) >= 5 * args.batch_size:
    #         train_dataset_list.append(dataset)
    #     else:
    #         small_datasets.append(dataset)
    # train_dataset_list.append(ConcatDataset(small_datasets))
    train_ds_list = [Rumor_Data(train) for train in train_list]
    train_ds_list = [dataset for dataset in train_ds_list if len(dataset) >= 2 * args.batch_size]  # filter small-event Datasets
    # train_dataset = Rumor_Data(train)
    validate_dataset = Rumor_Data(validation)
    test_dataset = Rumor_Data(test)

    # make replay Datasets: now dataset + 0.5 * random previous dataset
    train_dataset_list = [train_ds_list[0]]
    for dsi in range(1, len(train_ds_list)):
        dataset_previous_length = len(train_ds_list[dsi - 1])
        indices = torch.randperm(dataset_previous_length)[:int(0.25 * dataset_previous_length)]
        half_previous_dataset = Subset(train_ds_list[dsi - 1], indices)
        train_dataset_list.append(ConcatDataset([train_ds_list[dsi], half_previous_dataset]))

    # Data Loader (Input Pipeline)
    train_loader4test_list = [DataLoader(dataset=train_dataset, batch_size=args.batch_size, num_workers=args.number_workers, shuffle=True, drop_last=True)
                              for train_dataset in train_ds_list]
    ...
    for eventi, train_loader in enumerate(train_loader_list):
        ...
        for test_event_i, test_event_loader in enumerate(train_loader4test_list):
...
if __name__ == '__main__':
    ...
    args.id = '{}-{}-s{}-{}.log'.format(args.dataset, args.baseline + 'moereplay' if args.moe else args.baseline + 'replay',
